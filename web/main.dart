...
class App {
  ...
  Future<void> importFiles()async{final input=el<HTMLInputElement>('fileInput');final files=input.files;if(files==null)return;for(var i=0;i<files.length;i++){final f=files.item(i);if(f==null)continue;try{final now=DateTime.now(),id='import-${now.microsecondsSinceEpoch}-$i';final ext=f.name.contains('.')?f.name.split('.').last.toLowerCase():extensionFor(f.type);final mime=importMimeType(ext,f.type);final blob=f.type.isEmpty||!f.type.startsWith('audio/')?Blob([f].toJS,BlobPropertyBag(type:mime)):f;final meta={'id':id,'title':f.name.replaceFirst(RegExp(r'\.[^.]+$'),''),'createdAt':now.toUtc().toIso8601String(),'updatedAt':now.toUtc().toIso8601String(),'duration':0,'mimeType':mime,'fileExtension':ext,'size':blob.size,'storageKey':'recording-$id.$ext','favorite':false,'playCount':0};await bridgeSave(jsonEncode(meta),blob).toDart;}catch(_){toast('${f.name}を読み込めませんでした。');}}input.value='';await reload();await refreshStorage();toast('ファイルを読み込みました。');}
...
String importMimeType(String extension,String reported){if(reported.startsWith('audio/'))return reported;return switch(extension){'m4a'||'mp4'=>'audio/mp4','aac'=>'audio/aac','wav'||'wave'=>'audio/wav','mp3'=>'audio/mpeg','ogg'||'oga'=>'audio/ogg','webm'=>'audio/webm',_=>'application/octet-stream'};}
